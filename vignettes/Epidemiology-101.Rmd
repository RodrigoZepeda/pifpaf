...
Under this scenario, only `r cancer_cft_2` individuals would develop lung cancer (vs `r cancer` in the real population). So `r cancer` - `r cancer_cft_2` = `r cancer - cancer_cft_2` cases would be avoided if exposure was reduced to the minimum risk (in this case, not smoking). Those `r cancer - cancer_cft_2` cases represent `r scales::percent((cancer - cancer_cft_2)/cancer)`of all cases. Thus the potential impact fraction of reducing the smoker population by 40\% would be `r scales::percent((cancer - cancer_cft_2)/cancer)`. That is, `r scales::percent((cancer - cancer_cft_2)/cancer)` of all lung cancer cases in this population would be avoided if we reduced the amount of smokers by 40%.
